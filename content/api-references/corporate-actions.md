...
| `ex_date`          | The first date that any purchases of a security will not result in a corporate action entitlement.                  |
| `record_date`      | The date that an account must hold a position in the security in order to receive the corporate action entitlement. |
...
## **Retrieving a Corporate Action Announcement**

`GET /v1/corporate_actions/announcements/{id}`

This enables searching for a specific corporate action announcement by specifying the announcement's ID.
